...
contract FmuldivTest is DSTest {
    ...
    function testFMul() public {
        ...
        uint result2;
        uint result3;
        (, bytes memory result) = fmul.staticcall(abi.encode(0.5e27, 2.5e27, 1e27));
        assembly { result1 := mload(add(result, 32)) }
        (, result) = fmul.staticcall(abi.encode(0.5e18, 2.5e18, 1e18));
        assembly { result2 := mload(add(result, 32)) }
        (, result) = fmul.staticcall(abi.encode(0.5e8, 2.5e8, 1e8));
        assembly { result3 := mload(add(result, 32)) }
    ...
    function testFDiv() public {
        ...
        uint result2;
        uint result3;
        (, bytes memory result) = fdiv.staticcall(abi.encode(1e27, 2e27, 1e27));
        assembly { result1 := mload(add(result, 32)) }
        (, result) = fdiv.staticcall(abi.encode(1e18, 2e18, 1e18));
        assembly { result2 := mload(add(result, 32)) }
        (, result) = fdiv.staticcall(abi.encode(1e8, 2e8, 1e8));
        assembly { result3 := mload(add(result, 32)) }
